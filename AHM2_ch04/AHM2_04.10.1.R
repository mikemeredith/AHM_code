...
op <- par(mfrow = c(1, 3), mar = c(5,5,5,3), cex.axis = 1.5, cex.lab = 1.5, cex.main = 1.6)

# (1) Plot realised and apparent proportion of occupied sites
year <- 1:data$nyear
plot(year, data$psi.fs, type = "l", xlab = "Year", ylab = "Occupancy probability", col = "red",
...
summary(lm(data$psi.fs ~ year))
summary(lm(data$psi.app ~ year))

# (2) Estimates from the two models
plot(year-0.1, out0$mean$psi, type = "b", xlab = "Year", ylab = "Occupancy probability",
    col = "blue", ylim = c(0.35,0.8), lwd = 2, lty = 1, frame.plot = FALSE, las = 1,
    main = 'Estimated occupancy with and \nwithout detection heterogeneity')
segments(1:data$nyear-0.1, out0$q2.5$psi, 1:data$nyear-0.1, out0$q97.5$psi, col = "blue", lwd=1)
points(1:data$nyear+0.1, out$mean$psi, col = "brown", type = "b", lwd = 2, pch = 16)
segments(1:data$nyear+0.1, out$q2.5$psi, 1:data$nyear+0.1, out$q97.5$psi, col = "brown", lwd=1)
legend('bottomleft', c('Heterogeneity model', 'Simple model'), col = c('brown', 'blue'),
    lwd = 3, bty = 'n', cex = 1.3)

# (3) Estimated magnitude of detection heterogeneity
boxplot(out$sims.list$sd.eps ~ matrix(rep(1:data$nyear, out$mcmc.info$n.samples),
    ncol = data$nyear, byrow = T), col = "gold", outline = F, xlab = "Year",
    ylab = "Heterogeneity SD", frame = F, las = 1,
    main = 'True and estimated \nsite-level detection heterogeneity')
lines(1:data$nyear, data$sd.site, col = "red", lwd = 2)
par(op)
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
