...
# ~~~~ code for model 3 from MS dated 2019-01-02 ~~~~~~~~~~~~~~~~~~~~~~~~~~
...
model {
  ...
  # Model for phi and gamma: cliff height + smooth random year effects
  for (i in 1:nsites){
    for(t in 1:(nyears-1)){
      logit(phi[i,t]) <- lphi[i,t]
      lphi[i,t] <- alpha.lphi[height[i]] + lphi.year[t]
      logit(gamma[i,t]) <- lgamma[i,t]
      lgamma[i, t] <- alpha.lgamma[height[i]] + lgamma.year[t]
    }
  }
  # Priors for phi and gamma intercepts
  for(k in 1:3){
...
plot(year, n.occ.obs, xlab = 'Year', ylab = 'Population size',
    cex = 2, type = 'b', pch = 16, ylim = c(0, 230), frame = FALSE)
points(year, n.ratio, cex = 2, pch = 1, type = 'b', col = 'blue')
...
matplot(cbind(out1$mean$mean.phi.year, out2$mean$mean.phi.year,
    out3$mean$mean.phi.year, out1$mean$mean.gamma.year,
    out2$mean$mean.gamma.year, out3$mean$mean.gamma.year),
    col = c(rep('red', 3), rep('blue', 3), type = 'b'))
...
plot(year[-53], out1$mean$mean.phi.year, xlab = 'Year', ylab = 'Probability',
    cex = 2, type = 'b', pch = 0, ylim = c(0, 1), frame = FALSE)
points(year[-53], phi.ma, cex = 2, pch = 15, type = 'b')
points(year[-53], out1$mean$mean.gamma.year, cex = 2, pch = 1, type = 'b')
points(year[-53], gamma.ma, cex = 2, pch = 16, type = 'b')
legend(1964, 0.5, c('Persistence (no PS)', 'Persistence (with PS)',
    'Colonization (no PS)', 'Colonization (with PS)'),
    lty = 1, pch = c(0,15,1,16), bty = 'n', cex = 1.5)
par(op)
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

# Model-averaged phi and gamma per cliff-height category
# ~~~~~~~~~ inserted code from MS dated2019-01-02 ~~~~~~~~~~~~~~~
...
cliff.tab <- cbind('post.mean phi' = apply(p.cl, 2, mean),
    'CRI' = t(apply(p.cl, 2, function(x) quantile(x, c(0.025, 0.975)))),
    'post.mean gamma' = apply(g.cl, 2, mean),
    'CRI' = t(apply(g.cl, 2, function(x) quantile(x, c(0.025, 0.975)))))
