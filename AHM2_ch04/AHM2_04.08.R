...
str(bdata <- list(y = data$y, nsite = data$nsite, nyear = data$nyear, nsurvey = data$nsurvey, Xpsi1 = data$Xpsi1, Xphi = data$Xphi, Xgamma = data$Xgamma, Xp = data$Xp, e = 0.0001))
...
params <- c('mean.psi1', 'beta.lpsi', 'mean.phi', 'beta.lphi', 'mean.gamma', 'beta.lgamma', 'mean.p', 'beta.lp', 'Chi2Open', 'Chi2repOpen', 'Chi2ratioOpen', 'Chi2Closed', 'Chi2repClosed', 'Chi2ratioClosed', 'FTClosed', 'FTrepClosed', 'FTratioClosed', 'tm', 'tmrep', 'Etm')
...
par(mfrow = c(3,3))    ;    traceplot(out.gof)
print(out.gof, 2)
...
par(mfrow = c(1,3), mar = c(5,5,5,4), cex.lab = 1.5, cex.axis = 1.5, cex.main = 1.5)
